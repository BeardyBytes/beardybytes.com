...
const numberedSpans = (property, entries) =>
  entries
    .map((entry) => entry[property])
    .map((num) => (num === null || num == undefined ? ' ' : num))
    .map((num) => `<span>${num}</span>`)

const markEntries = (entries) =>
  entries
...
function blocksToHtml(language, blocks) {
  const entries = blocks.map(extractBlockEntries).reduce((acc, curr) => acc.concat(curr), [])

  const codeHtml = blocks.map((block) => highlightBlock(language, block)).join('\n')
